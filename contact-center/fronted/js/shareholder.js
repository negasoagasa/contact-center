document.addEventListener('DOMContentLoaded', function() {
    ...
    if (currentUser) {
        document.getElementById('currentShareholderName').textContent = currentUser.name;
    }
    ...
    // Form submission
    const responseForm = document.getElementById('shareholderResponseForm');
    if (responseForm) {
        responseForm.addEventListener('submit', function(e) {
            e.preventDefault();
            submitDepartmentResponse('shareholder');
        });
    }
    ...
    // Close modal
    const closeModal = document.querySelector('#shareholderResponseModal .close-modal');
    if (closeModal) {
        closeModal.addEventListener('click', function() {
            document.getElementById('shareholderResponseModal').style.display = 'none';
        });
    }
...
function updateDashboardStats() {
    ...
    const escalatedCalls = calls.filter(call => call.status === 'escalated' && call.escalatedTo === 'shareholder');
    const pendingCalls = calls.filter(call => call.status === 'pending' && call.escalatedTo === 'shareholder');
    const solvedToday = calls.filter(call => call.status === 'solved' && call.escalatedTo === 'shareholder' && call.date === today);
    
    document.getElementById('shareholderTotalEscalated').textContent = escalatedCalls.length;
    document.getElementById('shareholderPendingResponse').textContent = pendingCalls.length;
    document.getElementById('shareholderSolvedToday').textContent = solvedToday.length;
}
...
function loadEscalatedCalls() {
    ...
    const escalatedCalls = calls.filter(call => 
        ...
        call.escalatedTo === 'shareholder'
    ).reverse();
    
    renderCallTable('#shareholderEscalatedTable', escalatedCalls);
}
...
function loadRecentEscalatedCalls() {
    ...
    renderCallTable('#shareholderRecentEscalatedTable', escalatedCalls);
}
...
function openResponseModal(callId) {
    const modal = document.getElementById('shareholderResponseModal');
    if (!modal) return;
    
    document.getElementById('shareholderResponseCallId').value = callId;
    document.getElementById('shareholderResponseText').value = '';
    modal.style.display = 'block';
}
...
function submitDepartmentResponse(department) {
    ...
    const callId = parseInt(document.getElementById('shareholderResponseCallId').value);
    const responseText = document.getElementById('shareholderResponseText').value.trim();
