document.addEventListener('DOMContentLoaded', function() {
    ...
    if (currentUser) {
        document.getElementById('currentDigitalName').textContent = currentUser.name;
    }
    ...
    // Form submission
    const responseForm = document.getElementById('digitalResponseForm');
    if (responseForm) {
        responseForm.addEventListener('submit', function(e) {
            e.preventDefault();
            submitDepartmentResponse('digital');
        });
    }
    ...
    // Close modal
    const closeModal = document.querySelector('#digitalResponseModal .close-modal');
    if (closeModal) {
        closeModal.addEventListener('click', function() {
            document.getElementById('digitalResponseModal').style.display = 'none';
        });
    }
...
function updateDashboardStats() {
    ...
    const escalatedCalls = calls.filter(call => call.status === 'escalated' && call.escalatedTo === 'digital');
    const pendingCalls = calls.filter(call => call.status === 'pending' && call.escalatedTo === 'digital');
    const solvedToday = calls.filter(call => call.status === 'solved' && call.escalatedTo === 'digital' && call.date === today);
    
    document.getElementById('digitalTotalEscalated').textContent = escalatedCalls.length;
    document.getElementById('digitalPendingResponse').textContent = pendingCalls.length;
    document.getElementById('digitalSolvedToday').textContent = solvedToday.length;
}
...
function loadEscalatedCalls() {
    ...
    const escalatedCalls = calls.filter(call => 
        ...
        call.escalatedTo === 'digital'
    ).reverse();
    
    renderCallTable('#digitalEscalatedTable', escalatedCalls);
}
...
function loadRecentEscalatedCalls() {
    ...
    renderCallTable('#digitalRecentEscalatedTable', escalatedCalls);
}
...
function openResponseModal(callId) {
    const modal = document.getElementById('digitalResponseModal');
    if (!modal) return;
    
    document.getElementById('digitalResponseCallId').value = callId;
    document.getElementById('digitalResponseText').value = '';
    modal.style.display = 'block';
}
...
function submitDepartmentResponse(department) {
    ...
    const callId = parseInt(document.getElementById('digitalResponseCallId').value);
    const responseText = document.getElementById('digitalResponseText').value.trim();
